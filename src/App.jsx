...
function App() {
    ...
    if (loading) return <div className="h-screen flex items-center justify-center font-black text-blue-900 animate-pulse uppercase">Carregando Lopes Prime...</div>;
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-20 font-sans text-gray-900">
            ...
            <nav className="bg-white border-b sticky top-16 z-40 flex flex-wrap justify-center gap-1 p-2 shadow-md">
                <button onClick={() => {setActiveTab('clients'); resetForm();}} className={`py-2 px-3 rounded-xl font-black text-[9px] uppercase ${activeTab === 'clients' ? 'bg-blue-900 text-white shadow-lg' : 'bg-gray-100 text-gray-400'}`}>👥 Clientes</button>
                <button onClick={() => {setActiveTab('properties'); resetForm();}} className={`py-2 px-3 rounded-xl font-black text-[9px] uppercase ${activeTab === 'properties' ? 'bg-blue-900 text-white shadow-lg' : 'bg-gray-100 text-gray-400'}`}>🏠 Imóveis</button>
                <button onClick={() => {setActiveTab('add-client'); resetForm();}} className="py-2 px-3 rounded-xl font-black text-[9px] uppercase bg-green-100 text-green-700 border border-green-200">➕ Novo Cliente</button>
                <button onClick={() => {setActiveTab('add-property'); resetForm();}} className="py-2 px-3 rounded-xl font-black text-[9px] uppercase bg-purple-100 text-purple-700 border border-purple-200">➕ Novo Imóvel</button>
            </nav>
            ...
            <main className="max-w-7xl mx-auto p-4 mt-4">
                {activeTab === 'clients' && (
                    <>
                        <div className="max-w-2xl mx-auto mb-8 space-y-4">
                            <input type="text" placeholder="🔍 Pesquisar por nome ou imóvel..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full p-4 bg-white border-2 rounded-2xl font-bold shadow-md outline-none focus:border-blue-500" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {filteredClients.map(c => (
                                <div key={c.id} className="bg-white rounded-[2rem] shadow-lg p-6 border-t-8 border-blue-900 relative flex flex-col hover:shadow-2xl transition duration-300">
                                    {editingId !== c.id && (
                                        <button onClick={() => startEditClient(c)} className="absolute top-3 left-3 z-10 text-gray-400 hover:text-blue-600 p-2 bg-gray-50 rounded-full shadow-sm">✏️</button>
                                    )}
                                    ...
                                    {editingId === c.id ? (
                                        <div className="space-y-3">
                                            ...
                                            <input type="date" value={birthDate} onChange={e => setBirthDate(e.target.value)} className="w-full p-2 border-2 rounded-lg text-sm font-bold uppercase" />
                                            ...
                                            <div className="flex gap-2 pt-2">
                                                <button onClick={() => saveEditClient(c.id)} className="flex-1 bg-green-500 text-white font-black py-2 rounded-xl text-xs uppercase">Salvar</button>
                                        ...
                                        <>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 mb-4 text-[9px] font-bold uppercase">
                                                ...
                                                <div className="bg-gray-50 p-2 rounded-lg leading-tight"><p className="text-green-700 text-[7px]">📞 WhatsApp</p>{c.phones?.[0]}</div>
                                            </div>
                                            <div className="mb-4 bg-gray-50 p-3 rounded-xl border border-gray-100 h-24 overflow-y-auto text-xs italic font-medium text-gray-600 scrollbar-hide">
                                                {c.observations || 'Sem anotações de atendimento.'}
                                            </div>
                                            <div className="mt-auto space-y-2">
                                                <select value={c.status || 'LEAD'} onChange={(e) => updateStatus(c.id, e.target.value)} className="w-full p-2.5 bg-blue-50 border-2 border-blue-100 rounded-xl text-[10px] font-black text-blue-900 outline-none uppercase">
                                                ...
                                                <button onClick={() => deleteDoc(doc(db, 'clients', c.id)).then(() => loadData(user.uid))} className="w-full text-[9px] font-black text-red-200 hover:text-red-500 uppercase transition tracking-widest text-center mt-2">Remover do CRM</button>
                                            </div>
                                        </>
                                    )}
                                </div>
                ...
                {activeTab === 'properties' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {properties.map(p => (
                            <div key={p.id} className="bg-white rounded-[2rem] shadow-xl overflow-hidden border border-gray-100 relative flex flex-col hover:shadow-2xl transition duration-300">
                                {editingId !== p.id && (
                                ...
                                {editingId === p.id ? (
                                    <div className="p-6 space-y-3">
                                        ...
                                        <textarea value={observations} onChange={e => setObservations(e.target.value)} className="w-full p-2 border-2 rounded-lg text-xs h-20" placeholder="Descrição" />
                                        <div className="flex gap-2 pt-2">
                                            ...
                                            <button onClick={() => setEditingId(null)} className="flex-1 bg-gray-200 text-gray-600 font-black py-2 rounded-xl text-xs uppercase">Sair</button>
                                        </div>
                                    </div>
                                ) : (
                                    <>
                                        ...
                                        <div className="p-6 flex flex-col flex-grow">
                                            ...
                                            <p className="text-[10px] font-bold text-gray-400 uppercase mb-5 leading-tight italic">📍 {p.address || 'Local não informado'}</p>
                                            ...
                                            <div className="flex flex-col gap-3 mt-auto">
                                                ...
                                                <button onClick={() => deleteDoc(doc(db, 'properties', p.id)).then(() => loadData(user.uid))} className="mt-4 text-[9px] font-black text-gray-300 hover:text-red-600 uppercase transition tracking-widest text-center">Excluir Imóvel</button>
                                            </div>
                                        </div>
                                    </>
                                )}
                ...
                {(activeTab === 'add-client' || activeTab === 'add-property') && (
                    <div className="max-w-xl mx-auto bg-white p-10 rounded-[3rem] shadow-2xl border-2 border-gray-50">
                        <h2 className={`text-2xl font-black mb-8 text-center uppercase italic tracking-tighter ${activeTab === 'add-client' ? 'text-blue-900' : 'text-purple-900'}`}>
                            {activeTab === 'add-client' ? 'Novo Cadastro de Cliente' : 'Novo Cadastro de Imóvel'}
                        </h2>
                        
                        <div className="space-y-4">
                            <input type="text" placeholder="Nome Completo / Título" value={name} onChange={e => setName(e.target.value)} className="w-full p-4 bg-gray-50 rounded-2xl font-bold border-2 border-transparent focus:border-blue-500 outline-none transition shadow-inner" />
                            
                            {activeTab === 'add-client' ? (
                                <>
                                    <input type="text" placeholder="WhatsApp (219XXXXXXXX)" value={phone} onChange={e => setPhone(e.target.value)} className="w-full p-4 bg-gray-50 rounded-2xl font-bold border-2 border-transparent focus:border-blue-500 outline-none transition shadow-inner" />
                                    ...
                                    <select value={propertyInterest} onChange={e => setPropertyInterest(e.target.value)} className="w-full p-4 bg-yellow-50 rounded-2xl font-black border-2 border-yellow-100 outline-none focus:border-yellow-400 transition">
                                        <option value="">Selecione o Imóvel de Interesse...</option>
                                        {properties.map(p => <option key={p.id} value={p.title}>{p.title}</option>)}
                                    </select>
                                    <textarea placeholder="Observações do atendimento..." value={observations} onChange={e => setObservations(e.target.value)} className="w-full p-4 bg-gray-50 rounded-2xl font-bold border-2 border-transparent focus:border-blue-500 outline-none transition h-32 shadow-inner" />
                                </>
                            ) : (
                                <>
                                    <input type="text" placeholder="Preço Sugerido" value={propPrice} onChange={e => setPropPrice(formatCurrency(e.target.value))} className="w-full p-4 bg-green-50 rounded-2xl font-black text-green-700 border-2 border-green-100 shadow-inner outline-none focus:border-green-500 transition" />
                                    <input type="text" placeholder="Links das Fotos (URL, URL...)" value={propImg} onChange={e => setPropImg(e.target.value)} className="w-full p-4 bg-gray-50 rounded-2xl font-bold border-2 border-transparent focus:border-blue-500 outline-none transition shadow-inner text-xs italic" />
                                    <input type="text" placeholder="Link do Site Oficial" value={propLink} onChange={e => setPropLink(e.target.value)} className="w-full p-4 bg-blue-50 rounded-2xl font-bold border-2 border-transparent focus:border-blue-500 outline-none transition shadow-inner" />
                                    <input type="text" placeholder="Link da Tabela de Preços (PDF)" value={propPdf} onChange={e => setPropPdf(e.target.value)} className="w-full p-4 bg-red-50 rounded-2xl font-bold border-2 border-transparent focus:border-red-500 outline-none transition shadow-inner" />
                                    <input type="text" placeholder="Endereço Completo" value={propAddress} onChange={e => setPropAddress(e.target.value)} className="w-full p-4 bg-gray-50 rounded-2xl font-bold border-2 border-transparent focus:border-blue-500 outline-none transition shadow-inner" />
                                </>
                            )}

                            <div className="flex gap-4 pt-4">
                                <button onClick={activeTab === 'add-client' ? addClient : addProperty} className={`flex-1 ${activeTab === 'add-client' ? 'bg-blue-900' : 'bg-purple-900'} text-white font-black py-5 rounded-3xl shadow-2xl uppercase tracking-widest text-lg transition hover:bg-black active:scale-95`}>Salvar</button>
                                <button onClick={() => {setActiveTab(activeTab === 'add-client' ? 'clients' : 'properties'); resetForm();}} className="flex-1 bg-gray-100 text-gray-400 font-black py-5 rounded-3xl shadow-sm uppercase tracking-widest text-lg transition hover:bg-gray-200">Cancelar</button>
                            </div>
                        </div>
                    </div>
                )}
