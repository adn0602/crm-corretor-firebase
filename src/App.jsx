...
const TailwindStyle = ({ theme }) => (
  <style>{`
    ...
    .glass-panel { background: ${theme.name.includes('Dark') ? 'rgba(30, 41, 59, 0.7)' : 'rgba(255, 255, 255, 0.9)'}; border: 1px solid ${theme.name.includes('Dark') ? 'rgba(255,255,255,0.1)' : '#e2e8f0'}; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); border-radius: 1.5rem; transition: all 0.3s ease; backdrop-filter: blur(10px); }
    .glass-panel:hover { transform: translateY(-3px); box-shadow: 0 15px 30px -5px rgba(0, 0, 0, 0.15); border-color: var(--primary); }
    .glass-column { background: ${theme.name.includes('Dark') ? 'rgba(15, 23, 42, 0.5)' : '#f8fafc'}; border: 1px solid ${theme.name.includes('Dark') ? 'rgba(255,255,255,0.05)' : '#e2e8f0'}; border-radius: 1.5rem; display: flex; flex-direction: column; }
        ...
        <div className="min-h-screen flex">
            ...
            <main className="flex-1 ml-20 lg:ml-64 p-6 lg:p-10 overflow-y-auto transition-colors duration-500">
                <header className="flex justify-between items-center mb-8 animate-fadeIn no-print">
                    <div><h2 className="text-xs font-black opacity-50 uppercase tracking-widest mb-1">{greeting}, {userProfile.name}</h2><h1 className="text-3xl font-black tracking-tight uppercase">{activeTab === 'settings' ? 'Configurações' : activeTab}</h1></div>
                ...
                {activeTab === 'dashboard' && (
                    <div className="space-y-8 animate-fadeIn">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            ...
                            <div className="glass-panel p-6 border-l-4 border-primary"><p className="opacity-50 text-xs font-bold uppercase mb-2">Vendas Fechadas</p><p className="text-3xl font-black text-green-600">{salesCount}</p></div>
                        </div>
                    ...
                    <div className="space-y-8 animate-fadeIn">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div className="glass-panel p-8 flex flex-col justify-center items-center text-center">
                                <div className="w-32 h-32 rounded-full bg-blue-100 flex items-center justify-center text-4xl mb-6">🏆</div>
                                <h3 className="text-2xl font-black uppercase mb-2">Meta do Mês</h3>
                                <p className="text-sm opacity-60 mb-6 max-w-xs">Mantenha o foco, {userProfile.name}! Você tem <strong className="text-primary">{clients.filter(c => c.status === 'PROPOSTA').length}</strong> negociações em andamento com alto potencial de fechamento.</p>
                                <button onClick={() => setActiveTab('pipeline')} className="btn-primary px-8 py-3 rounded-xl text-xs uppercase shadow-lg">Ver Oportunidades</button>
                            </div>
                        </div>
                    </div>
                )}
