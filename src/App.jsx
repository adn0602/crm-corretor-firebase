...
// --- ESTILOS GLOBAIS DINÂMICOS (COM SCROLLBAR CORRIGIDA) ---
const TailwindStyle = ({ theme }) => {
  // Lógica de Contraste Automático
  const isDark = theme.name.includes('Midnight') || theme.name.includes('Cyber') || theme.name.includes('Future') || theme.bg.includes('#0f172a') || theme.bg.includes('#111827');
  
  // Variáveis de Contraste
  const panelBg = isDark ? 'rgba(30, 41, 59, 0.95)' : 'rgba(255, 255, 255, 0.98)';
  const panelBorder = isDark ? 'rgba(255, 255, 255, 0.1)' : '#e2e8f0';
  const panelText = isDark ? '#f1f5f9' : '#1e293b'; 
  const mutedText = isDark ? '#94a3b8' : '#64748b';
  const inputBg = isDark ? 'rgba(0, 0, 0, 0.4)' : '#f1f5f9';
  const sidebarBg = theme.sidebar;
  const sidebarText = isDark ? '#cbd5e1' : '#64748b';
  ...
  <style>{`
    ...
    .sidebar-link { transition: all 0.2s; border-radius: 1rem; font-weight: 700; font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.05em; display: flex; align-items: center; gap: 1rem; padding: 1rem; color: ${sidebarText}; }
    .sidebar-link.active { background-color: var(--primary); color: white; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.2); }
    .sidebar-link:hover:not(.active) { background-color: rgba(0,0,0,0.05); color: var(--panel-text); }
    ...
    .btn-back { background: rgba(0,0,0,0.1); color: var(--panel-text); font-weight: 800; }
    ...
    .btn-crm-outline { background-color: transparent; border: 1px solid var(--primary); color: var(--primary); border-radius: 0.25rem; font-weight: 600; padding: 0.5rem 1rem; transition: all 0.2s; }
    .btn-crm-outline:hover { background-color: var(--primary); color: white; }

    .btn-crm-ghost { background-color: transparent; border: 1px solid var(--panel-border); color: var(--muted-text); border-radius: 0.25rem; font-weight: 600; padding: 0.5rem 1rem; transition: all 0.2s; display: flex; items-center; gap: 0.5rem; }
    .btn-crm-ghost:hover { border-color: var(--primary); color: var(--panel-text); }
    ...
    .settings-input { 
        ...
        font-weight: 600; outline: none; transition: 0.2s; 
    }
    ...
    .kanban-card { 
        ...
        border: 1px solid var(--panel-border); 
        color: var(--panel-text);
        border-radius: 1rem; padding: 1.25rem; cursor: pointer; position: relative; margin-bottom: 0.75rem; 
    }
    .kanban-card:hover { border-color: var(--primary); transform: translateY(-3px); }

    .crm-table { width: 100%; border-collapse: separate; border-spacing: 0; color: var(--panel-text); }
    .crm-table th { text-align: left; padding: 1rem; font-size: 0.7rem; font-weight: 800; text-transform: uppercase; color: var(--muted-text); border-bottom: 1px solid var(--panel-border); }
    .crm-table td { padding: 1rem; border-bottom: 1px solid var(--panel-border); font-size: 0.85rem; font-weight: 500; }
    .crm-table tr:hover td { background-color: rgba(0,0,0,0.02); }
    .crm-table tr:last-child td { border-bottom: none; }

    /* UTIL */
    .text-primary { color: var(--primary); }
    .bg-primary-light { background-color: ${isDark ? 'rgba(255,255,255,0.05)' : '#f1f5f9'}; }
    ...
    /* SCROLLBAR PERSONALIZADA - Corrigida para ser visível */
    ...
    .custom-checkbox { width: 1.1rem; height: 1.1rem; border-radius: 0.25rem; border: 2px solid #cbd5e1; appearance: none; cursor: pointer; transition: 0.2s; position: relative; background-color: white; }
    ...
    /* AGENDA ESPECÍFICA */
    .agenda-row { border-bottom: 1px solid var(--panel-border); min-height: 80px; position: relative; }
    .agenda-time-col { width: 80px; text-align: right; padding-right: 1rem; font-size: 0.7rem; color: var(--muted-text); font-weight: 700; transform: translateY(-8px); }
    ...
    /* TEXTOS */
    .text-contrast { color: var(--panel-text); }
    ...
    return (
        <div className="min-h-screen flex">
            ...
            <aside className="w-20 lg:w-64 sidebar-container flex flex-col fixed h-full z-50">
                <div className="p-8"><h1 className="font-black text-xl tracking-tighter italic" style={{color: theme.text}}>ALEXANDRE<span className="text-primary">CRM</span></h1></div>
            ...
            <main className="flex-1 ml-20 lg:ml-64 p-6 lg:p-10 overflow-y-auto transition-colors duration-500 h-screen flex flex-col">
                ...
                <header className="flex justify-between items-center mb-6 animate-fadeIn flex-shrink-0">
                    <div>
                        <h2 className="text-xs font-black opacity-50 uppercase tracking-widest mb-1 text-muted">Painel de Controle</h2>
                        <h1 className="text-3xl font-black tracking-tight uppercase text-contrast">{activeTab === 'settings' ? 'Configurações' : activeTab === 'properties' ? 'Estoque de Imóveis' : activeTab === 'clientes' ? 'Gestão de Contatos' : activeTab}</h1>
                    </div>
                ...
                {activeTab === 'dashboard' && (
                    <div className="space-y-8 animate-fadeIn overflow-y-auto">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            <div className="glass-panel p-6 border-l-4 border-primary"><p className="opacity-50 text-xs font-bold uppercase mb-2 text-contrast">Clientes</p><p className="text-3xl font-black text-contrast">{clients.length}</p></div>
                            <div className="glass-panel p-6 border-l-4 border-primary"><p className="opacity-50 text-xs font-bold uppercase mb-2 text-contrast">VGV Total</p><p className="text-2xl font-black text-contrast">{totalVGV.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}</p></div>
                            <div className="glass-panel p-6 border-l-4 border-primary"><p className="opacity-50 text-xs font-bold uppercase mb-2 text-contrast">Em Negociação</p><p className="text-3xl font-black text-contrast">{clients.filter(c => c.status === 'PROPOSTA' || c.status === 'AGENDADO').length}</p></div>
                            <div className="glass-panel p-6 border-l-4 border-primary"><p className="opacity-50 text-xs font-bold uppercase mb-2 text-contrast">Agenda</p><p className="text-3xl font-black text-contrast">{agenda.length}</p></div>
                        </div>
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="glass-panel p-6 lg:col-span-2 shadow-sm h-80"><h3 className="text-sm font-black uppercase opacity-50 mb-4 text-contrast">Funil de Vendas</h3><ResponsiveContainer width="100%" height="100%"><BarChart data={funnelData} layout="vertical" margin={{top:5, right:30, left:20, bottom:5}}><XAxis type="number" hide /><YAxis dataKey="name" type="category" width={80} tick={{fontSize:10, fontWeight:700, fill: theme.text}} axisLine={false} tickLine={false} /><Tooltip cursor={{fill: 'rgba(0,0,0,0.05)'}} /><Bar dataKey="value" radius={[0, 4, 4, 0]} barSize={30}>{funnelData.map((e, i) => <Cell key={i} fill={e.color} />)}</Bar></BarChart></ResponsiveContainer></div>
                            <div className="glass-panel p-6 h-80 overflow-y-auto"><h3 className="text-sm font-black uppercase opacity-50 mb-4 text-contrast">Últimos Clientes</h3><div className="space-y-3">{clients.slice(0, 5).map(c => (<div key={c.id} className="flex justify-between items-center p-3 bg-primary-light rounded-xl border border-dashed border-slate-200"><div><p className="font-bold text-xs uppercase text-contrast">{c.fullName}</p><p className="text-[10px] opacity-50 text-contrast">{c.status || 'LEAD'}</p></div></div>))}</div></div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="flex gap-6 overflow-x-auto pb-6 scrollbar-hide h-[calc(100vh-180px)] animate-fadeIn">
                        {[{ id: 'LEAD', label: 'Novos Leads' }, { id: 'AGENDADO', label: 'Visitas' }, { id: 'PROPOSTA', label: 'Propostas' }, { id: 'FECHADO', label: 'Fechados' }].map(col => (
                            <div key={col.id} className="glass-column min-w-[320px] shadow-sm">
                                <div className="p-5 border-b border-dashed border-slate-200 flex justify-between items-center sticky top-0 z-10"><h3 className="font-black uppercase text-sm opacity-60 text-contrast">{col.label}</h3><span className="bg-primary-light px-2 py-1 rounded-lg text-xs font-bold opacity-60 shadow-inner text-contrast">{clients.filter(c => (c.status || 'LEAD') === col.id).length}</span></div>
                                <div className="p-4 flex-1 overflow-y-auto space-y-3">
                                    {clients.filter(c => (c.status || 'LEAD') === col.id).map(client => (
                                        <div key={client.id} onClick={() => openEdit(client, 'client')} className="kanban-card group">
                                            <div className="flex justify-between items-start mb-3"><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-[10px] font-black border border-slate-200 text-slate-500">{client.fullName.charAt(0).toUpperCase()}</div><div><h4 className="font-black text-xs uppercase leading-tight truncate w-32 text-contrast">{client.fullName}</h4><p className="text-[9px] opacity-50 font-bold text-contrast">{client.phones?.[0] || '-'}</p></div></div><button onClick={(e) => deleteItem('clients', client.id, e)} className="text-slate-300 hover:text-red-500 px-1">✕</button></div>
                ...
                {activeTab === 'clientes' && (
                    <div className="space-y-6 animate-fadeIn h-full flex flex-col">
                       ...
                       <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-2">
                           <div className="relative w-full md:w-1/3">
                               <input 
                                    ...
                                    className="settings-input w-full pl-10 pr-4 py-2"
                               />
                       ...
                       <div className="glass-panel flex-1 overflow-hidden flex flex-col">
                               ...
                               <table className="crm-table w-full">
                                   ...
                                   <tbody>
                                       {clients.filter(c => c.fullName.toLowerCase().includes(searchTerm.toLowerCase())).map(client => (
                                           <tr key={client.id} className="group hover:bg-white/5 transition-colors cursor-default">
                                               ...
                                               <td>
                                                   <div className="flex items-center gap-3" onClick={() => openEdit(client, 'client')}>
                                                       <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-500 border border-slate-200">{client.fullName.charAt(0).toUpperCase()}</div>
                                                       <span className="font-bold cursor-pointer hover:text-blue-600 text-contrast">{client.fullName}</span>
                                                   </div>
                                               </td>
                                               ...
                                               <td><span className="text-muted text-xs">{client.interest || '-'}</span></td>
                                               <td className="text-muted text-xs">{client.createdAt ? new Date(client.createdAt).toLocaleDateString('pt-BR') : '-'}</td>
                                               <td>
                           ...
                           <div className="p-3 border-t border-slate-200 text-xs text-muted flex justify-between items-center">
                               ...
                               <div key={p.id} className="glass-panel overflow-hidden group flex flex-col">
                                   ...
                                   <div className="p-6 flex-1 flex flex-col">
                                       <div className="flex justify-between items-start mb-2">
                                           <div><p className="text-[10px] font-bold uppercase text-muted tracking-wider">{p.developer || 'Incorporadora'}</p><h3 className="font-black text-xl uppercase leading-tight text-contrast">{p.title}</h3></div>
                                           <div className="text-right"><p className="text-xl font-black text-primary" style={{color: theme.primary}}>{p.price}</p></div>
                                       </div>
                                       <p className="text-xs text-muted font-medium mb-4 flex items-center gap-1">📍 {p.address || 'Localização não informada'}</p>

                                       <div className="grid grid-cols-3 gap-2 mb-4">
                                           <div className="bg-primary-light rounded-lg p-2 text-center border border-slate-100"><span className="block text-lg">🛏️</span><span className="text-[10px] font-bold uppercase text-muted">{p.bedrooms || 0} Quartos</span></div>
                                           <div className="bg-primary-light rounded-lg p-2 text-center border border-slate-100"><span className="block text-lg">🚿</span><span className="text-[10px] font-bold uppercase text-muted">{p.bathrooms || 0} Banheiros</span></div>
                                           <div className="bg-primary-light rounded-lg p-2 text-center border border-slate-100"><span className="block text-lg">🚗</span><span className="text-[10px] font-bold uppercase text-muted">{p.garage || 0} Vagas</span></div>
                                       </div>

                                       <div className="bg-primary-light rounded-xl p-3 border border-slate-100 mb-4 space-y-2">
                                           <div className="flex justify-between border-b border-dashed border-slate-200 pb-2"><span className="text-[10px] font-bold uppercase text-muted">Início Obras</span><span className="text-[10px] font-bold text-contrast">{p.constructionStart ? new Date(p.constructionStart).toLocaleDateString('pt-BR') : '-'}</span></div>
                                           <div className="flex justify-between border-b border-dashed border-slate-200 pb-2"><span className="text-[10px] font-bold uppercase text-muted">Entrega</span><span className="text-[10px] font-bold text-contrast">{p.deliveryDate ? new Date(p.deliveryDate).toLocaleDateString('pt-BR') : '-'}</span></div>
                                           <div><span className="text-[10px] font-bold uppercase text-muted block mb-1">Fluxo de Pagamento</span><p className="text-xs font-bold text-contrast">{p.paymentPlan || 'Consulte condições'}</p></div>
                                       </div>
                    ...
                    <div className="flex gap-6 h-[calc(100vh-140px)] animate-fadeIn">
                        ...
                        <div className="w-64 flex-shrink-0 flex flex-col gap-6">
                            <div className="glass-panel p-6">
                                <h3 className="text-xs font-black uppercase opacity-50 mb-4 text-center text-contrast">Navegar</h3>
                                <div className="grid grid-cols-7 gap-1 text-center text-[10px] font-bold opacity-50 mb-2 text-contrast"><div>D</div><div>S</div><div>T</div><div>Q</div><div>Q</div><div>S</div><div>S</div></div>
                                <div className="grid grid-cols-7 gap-1">
                                    {generateCalendarDays().map((d, i) => (
                                        <div 
                                            key={i} 
                                            onClick={() => d && setSelectedDate(`2024-02-${String(d).padStart(2,'0')}`)} 
                                            className={`aspect-square flex items-center justify-center rounded-lg text-xs font-bold cursor-pointer transition ${selectedDate.endsWith(String(d).padStart(2,'0')) ? 'bg-primary text-white shadow-md' : d ? 'hover:bg-slate-100 text-contrast' : ''}`}
                        ...
                        <div className="glass-panel flex-1 flex flex-col overflow-hidden relative">
                            ...
                            <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-white/50 backdrop-blur-sm z-20">
                                <div>
                                    <h3 className="text-lg font-black uppercase text-contrast">Meu Calendário</h3>
                                    <p className="text-xs font-bold text-muted capitalize">{new Date(selectedDate).toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
                                </div>
                                            ...
                                            {agenda
                                                ...
                                                .map(event => {
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            <div>
                                                                <span className="text-[10px] font-bold opacity-60 block text-slate-800">{event.time}</span>
                ...
                {activeTab === 'whatsapp' && (
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fadeIn h-[calc(100vh-140px)]">
                       ...
                       <div className="glass-panel p-6 flex flex-col">
                           <div className="mb-4">
                               <h3 className="text-lg font-black uppercase mb-1 text-contrast">1. Selecionar Leads</h3>
                               <p className="text-xs text-muted mb-2">Escolha quem receberá a mensagem.</p>
                           ...
                           <div className="flex-1 overflow-y-auto space-y-2 pr-2 border-t border-slate-100 pt-4">
                               <div className="flex justify-between items-center mb-2 px-1">
                                    <span className="text-xs font-bold text-muted">{selectedClients.length} selecionados</span>
                               ...
                               {clients.filter(c => c.fullName.toLowerCase().includes(searchTerm.toLowerCase())).map(c => (
                                   <div key={c.id} onClick={() => toggleSelectClient(c.id)} className={`p-3 rounded-xl border flex items-center justify-between cursor-pointer transition ${selectedClients.includes(c.id) ? 'bg-primary-light border-primary' : 'bg-transparent border-slate-100 hover:border-blue-300'}`}>
                                       <div className="flex items-center gap-3">
                                           <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${selectedClients.includes(c.id) ? 'bg-primary text-white' : 'bg-slate-100 text-slate-400'}`} style={selectedClients.includes(c.id) ? {backgroundColor: theme.primary} : {}}>{c.fullName.charAt(0).toUpperCase()}</div>
                                           <div>
                                               <p className="text-xs font-bold uppercase text-contrast">{c.fullName}</p>
                                               <p className="text-[9px] text-muted">{c.phones?.[0] || 'Sem telefone'}</p>
                                           </div>
                                       </div>
                       ...
                       <div className="glass-panel p-6 flex flex-col">
                           <div className="mb-4">
                               <h3 className="text-lg font-black uppercase mb-1 text-contrast">2. Mensagem</h3>
                               <p className="text-xs text-muted">Escreva o texto único para todos.</p>
                           </div>
                           <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
                               {[{l:'Apresentação', t:'Olá! Tudo bem? Sou Alexandre e gostaria de apresentar oportunidades de imóveis.'}, {l:'Cobrar Visita', t:'Olá! Lembrete da nossa visita agendada para amanhã.'}, {l:'Promoção', t:'Oportunidade única! Imóvel com desconto especial essa semana.'}].map((t,i) => (
                                   <button key={i} onClick={() => setBulkMessage(t.t)} className="px-3 py-1 bg-primary-light text-muted rounded-full text-[10px] font-black uppercase whitespace-nowrap hover:bg-slate-200 border border-slate-200 transition">{t.l}</button>
                               ))}
                           </div>
                           <textarea value={bulkMessage} onChange={e => setBulkMessage(e.target.value)} className="settings-input w-full h-48 mb-4 resize-none" placeholder="Digite sua mensagem aqui..." />
                       ...
                       <div className="glass-panel p-6 flex-1 bg-slate-900 text-white flex flex-col relative overflow-hidden">
                           ...
                           <div className="relative z-10 flex flex-col h-full">
                               <div className="flex justify-between items-start mb-6">
                                   <div>
                                       <h3 className="text-lg font-black uppercase mb-1 text-white">3. Execução</h3>
                                       <p className="text-xs opacity-50 text-white">Envie um por um para evitar bloqueios.</p>
                                   </div>
                                   ...
                                   <div className="flex items-center gap-2 bg-white/10 p-2 rounded-lg">
                                        ...
                                        <span className="text-[10px] font-bold uppercase text-white">Modo Teste</span>
                                   </div>
                               </div>
                               ...
                               {isTestMode && (
                                   <div className="mb-4 p-3 bg-yellow-500/20 border border-yellow-500/50 rounded-lg">
                                       ...
                                       <p className="text-[9px] mt-1 opacity-70 text-white">Todos os disparos irão para este número.</p>
                                   </div>
                               )}
                               
                               {sendingQueue.length === 0 ? (
                                   <div className="flex-1 flex flex-col items-center justify-center opacity-30 text-center text-white">
                                   ...
                                   <>
                                        <div className="mb-6">
                                            <div className="flex justify-between text-xs font-bold uppercase mb-2 text-white">
                                                <span>Progresso</span>
                                            ...
                                            <p className="text-[10px] text-center mt-2 opacity-60 text-white">{currentQueueIndex} de {sendingQueue.length} enviados</p>
                                        </div>
                                ...
                                <>
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        <div className="glass-panel p-8 border-t-4 border-primary hover:scale-[1.02] transition-transform">
                                            <p className="opacity-50 text-xs font-bold uppercase mb-2 text-contrast">Taxa de Conversão</p>
                                            <p className="text-4xl font-black text-contrast">{conversionRate}%</p>
                                            <p className="text-[10px] mt-2 opacity-60 text-muted">{closedClients} fechamentos de {totalClients} leads</p>
                                        </div>
                                        <div className="glass-panel p-8 border-t-4 border-primary hover:scale-[1.02] transition-transform">
                                            <p className="opacity-50 text-xs font-bold uppercase mb-2 text-contrast">Ticket Médio (Estoque)</p>
                                            <p className="text-3xl font-black text-contrast">{averageTicket.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}</p>
                                            <p className="text-[10px] mt-2 opacity-60 text-muted">Baseado em {properties.length} imóveis</p>
                                        </div>
                                        <div className="glass-panel p-8 border-t-4 border-primary hover:scale-[1.02] transition-transform">
                                            <p className="opacity-50 text-xs font-bold uppercase mb-2 text-contrast">Potencial de Comissão (5%)</p>
                                            ...
                                            <p className="text-[10px] mt-2 opacity-60 text-muted">Se vender todo o estoque atual</p>
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                        <div className="glass-panel p-8 flex flex-col h-96">
                                            <h3 className="text-sm font-black uppercase opacity-50 mb-4 text-contrast">Distribuição de Leads</h3>
                                            <ResponsiveContainer>
                                                <PieChart>
                                        ...
                                        <div className="glass-panel p-8 flex flex-col h-96">
                                             <h3 className="text-sm font-black uppercase opacity-50 mb-6 text-contrast">Produtividade Geral</h3>
                                             <div className="space-y-6 flex-1 overflow-y-auto">
                                                 <div className="flex items-center justify-between p-4 bg-primary-light rounded-xl border border-slate-100">
                                                     <div className="flex items-center gap-4">
                                                         <div className="w-10 h-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-xl">📅</div>
                                                         <div><p className="font-black text-sm uppercase text-contrast">Total Agendamentos</p><p className="text-[10px] opacity-50 text-muted">Visitas e Reuniões</p></div>
                                                     </div>
                                                     <span className="text-2xl font-black text-contrast">{agenda.length}</span>
                                                 </div>

                                                 <div className="flex items-center justify-between p-4 bg-primary-light rounded-xl border border-slate-100">
                                                     <div className="flex items-center gap-4">
                                                         <div className="w-10 h-10 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center text-xl">🏠</div>
                                                         <div><p className="font-black text-sm uppercase text-contrast">Imóveis Captados</p><p className="text-[10px] opacity-50 text-muted">Estoque Ativo</p></div>
                                                     </div>
                                                     <span className="text-2xl font-black text-contrast">{properties.length}</span>
                                                 </div>

                                                 <div className="flex items-center justify-between p-4 bg-primary-light rounded-xl border border-slate-100">
                                                     <div className="flex items-center gap-4">
                                                         <div className="w-10 h-10 rounded-full bg-green-100 text-green-600 flex items-center justify-center text-xl">💰</div>
                                                         <div><p className="font-black text-sm uppercase text-contrast">VGV Total</p><p className="text-[10px] opacity-50 text-muted">Valor Geral de Vendas</p></div>
                                                     </div>
                                                     <span className="text-lg font-black text-contrast">{totalVGV.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 })}</span>
                                                 </div>
                                             </div>
                                        </div>
                                    </div>
                    ...
                    <div className="animate-fadeIn overflow-y-auto pb-10">
                        <div className="flex gap-4 mb-8 overflow-x-auto pb-2">
                            {['perfil', 'seguranca', 'aparencia', 'sistema'].map(tab => (
                                <button key={tab} onClick={() => setSettingsTab(tab)} className={`px-6 py-2 rounded-full font-bold uppercase text-xs transition ${settingsTab === tab ? 'btn-primary' : 'bg-transparent text-muted border border-slate-200'}`}>{tab}</button>
                            ))}
                        </div>
                        ...
                        {settingsTab === 'perfil' && (
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                <div className="glass-panel p-8 flex flex-col items-center text-center">
                                    ...
                                    <h3 className="text-xl font-black uppercase mb-1 text-contrast">{userProfile.name || 'Usuário'}</h3>
                                    <p className="text-xs font-bold opacity-50 uppercase mb-6 text-muted">Corretor de Imóveis</p>
                                    <button className="btn-primary px-6 py-2 rounded-xl text-xs uppercase w-full">Alterar Foto</button>
                                </div>
                                <div className="lg:col-span-2 glass-panel p-8">
                                    <h3 className="text-lg font-black uppercase mb-6 opacity-70 text-contrast">Dados Pessoais</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Nome Completo</label><input className="settings-input mt-1" value={userProfile.name} onChange={e => setUserProfile({...userProfile, name: e.target.value})} /></div>
                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">CRECI</label><input className="settings-input mt-1" value={userProfile.creci} onChange={e => setUserProfile({...userProfile, creci: e.target.value})} placeholder="00000-F" /></div>
                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Email (Login)</label><input className="settings-input mt-1 opacity-50 cursor-not-allowed" value={user.email} disabled /></div>
                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Telefone</label><input className="settings-input mt-1" value={userProfile.phone} onChange={e => setUserProfile({...userProfile, phone: e.target.value})} /></div>
                                        <div className="md:col-span-2"><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Bio / Apresentação</label><textarea className="settings-input mt-1 h-24" value={userProfile.bio} onChange={e => setUserProfile({...userProfile, bio: e.target.value})} placeholder="Especialista em imóveis de alto padrão..." /></div>
                                    </div>
                        ...
                        {settingsTab === 'seguranca' && (
                            <div className="max-w-2xl mx-auto glass-panel p-10">
                                <h3 className="text-xl font-black uppercase mb-2 text-contrast">Segurança da Conta</h3>
                                <p className="text-sm opacity-60 mb-8 text-muted">Gerencie sua senha e acesso ao sistema.</p>
                                <div className="space-y-6">
                                    <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Nova Senha</label><input type="password" class="settings-input mt-1" value={passwords.new} onChange={e => setPasswords({...passwords, new: e.target.value})} /></div>
                                    <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Confirmar Nova Senha</label><input type="password" class="settings-input mt-1" value={passwords.confirm} onChange={e => setPasswords({...passwords, confirm: e.target.value})} /></div>
                                </div>
                        ...
                        {settingsTab === 'aparencia' && (
                            <div className="glass-panel p-10">
                                <h3 className="text-xl font-black uppercase mb-2 text-contrast">Personalização Visual</h3>
                                <p className="text-sm opacity-60 mb-8 text-muted">Escolha um tema que combine com seu estilo de trabalho.</p>
                        ...
                        {settingsTab === 'sistema' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="glass-panel p-8">
                                    <h3 className="text-lg font-black uppercase mb-6 opacity-70 text-contrast">Informações da Build</h3>
                                    <div className="space-y-4">
                                        <div className="flex justify-between py-3 border-b border-dashed border-slate-200"><span className="text-xs font-bold opacity-50 uppercase text-muted">Versão do CRM</span><span className="text-xs font-black text-contrast">v2.5.0 (Gold)</span></div>
                                        <div className="flex justify-between py-3 border-b border-dashed border-slate-200"><span className="text-xs font-bold opacity-50 uppercase text-muted">ID da Licença</span><span className="text-xs font-black font-mono text-contrast">PRO-8829-XJ</span></div>
                                        <div className="flex justify-between py-3 border-b border-dashed border-slate-200"><span className="text-xs font-bold opacity-50 uppercase text-muted">Engine</span><span className="text-xs font-black text-contrast">React 18 + Firebase 9</span></div>
                                        <div className="flex justify-between py-3"><span className="text-xs font-bold opacity-50 uppercase text-muted">Canal de Atualização</span><span className="text-xs font-bold text-green-500 bg-green-50 px-2 py-0.5 rounded">Stable</span></div>
                                    </div>
                                </div>
                                <div className="glass-panel p-8">
                                    <h3 className="text-lg font-black uppercase mb-6 opacity-70 text-contrast">Diagnóstico de Rede</h3>
                                    <div className="space-y-6">
                                        <div><div className="flex justify-between mb-1"><span className="text-[10px] font-bold uppercase opacity-50 text-contrast">Latência do Banco de Dados</span><span className="text-[10px] font-bold text-green-500">24ms (Excelente)</span></div><div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden"><div className="bg-green-500 h-full w-[15%]"></div></div></div>
                                        <div><div className="flex justify-between mb-1"><span className="text-[10px] font-bold uppercase opacity-50 text-contrast">Uso de Armazenamento</span><span className="text-[10px] font-bold text-contrast">1.2GB / 50GB</span></div><div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden"><div className="bg-blue-500 h-full w-[5%]"></div></div></div>
                                        <div className="p-4 bg-primary-light rounded-xl border border-slate-200"><p className="text-xs font-bold opacity-60 uppercase mb-2 text-contrast">Logs do Sistema</p><div className="font-mono text-[10px] opacity-50 space-y-1 text-muted"><p>> System initialized at {new Date().toLocaleTimeString()}</p><p>> Connected to Firestore [South America]</p><p>> Auth Token Verified: OK</p></div></div>
                                    </div>
                                </div>
                            </div>
                        )}
            ...
            {/* MODAL GLOBAL (COM SCROLLBAR CORRIGIDA E ESTRUTURA FIXA) */}
            {showForm && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 animate-fadeIn">
                    <div className="glass-panel w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden relative shadow-2xl">
                        
                        {/* 1. CABEÇALHO FIXO */}
                        <div className="p-6 pb-4 flex-shrink-0 border-b border-slate-200/10 z-10 bg-inherit rounded-t-2xl">
                            <h3 className="text-xl font-black uppercase text-contrast">{editingId ? 'Editar Registro' : 'Novo Registro'}</h3>
                        </div>
                        
                        {/* 2. CORPO COM SCROLL (O "Recheio") */}
                        <div className="px-6 py-4 overflow-y-auto custom-scrollbar flex-1">
                            <div className="space-y-4 pb-2">
                                {/* CAMPOS COMUNS */}
                                {(formData.type === 'client' || formData.type === 'agenda') && (
                                    <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Nome / Título</label><input className="settings-input mt-1" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} /></div>
                                )}
                                ...
                                {formData.type === 'client' && (
                                    <>
                                        <div className="grid grid-cols-2 gap-4">
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Telefone / Whats</label><input className="settings-input mt-1" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} /></div>
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Nascimento</label><input type="date" className="settings-input mt-1" value={formData.birthDate} onChange={e => setFormData({...formData, birthDate: e.target.value})} /></div>
                                        </div>
                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Email</label><input className="settings-input mt-1" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} /></div>
                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Imóvel Desejado</label><input className="settings-input mt-1" value={formData.interest} onChange={e => setFormData({...formData, interest: e.target.value})} /></div>
                                    </>
                                )}
                                ...
                                {formData.type === 'property' && (
                                    <div className="space-y-4">
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="col-span-2"><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Nome do Empreendimento</label><input className="settings-input mt-1" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} /></div>
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Incorporadora</label><input className="settings-input mt-1" value={formData.developer} onChange={e => setFormData({...formData, developer: e.target.value})} /></div>
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Preço (A partir de)</label><input className="settings-input mt-1" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} /></div>
                                        </div>
                                        
                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Endereço do Imóvel</label><input className="settings-input mt-1" value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} /></div>

                                        <div className="grid grid-cols-4 gap-2">
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Quartos</label><input type="number" className="settings-input mt-1" value={formData.bedrooms} onChange={e => setFormData({...formData, bedrooms: e.target.value})} /></div>
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Banheiros</label><input type="number" className="settings-input mt-1" value={formData.bathrooms} onChange={e => setFormData({...formData, bathrooms: e.target.value})} /></div>
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Vagas</label><input type="number" className="settings-input mt-1" value={formData.garage} onChange={e => setFormData({...formData, garage: e.target.value})} /></div>
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Unidades</label><input type="number" className="settings-input mt-1" value={formData.units} onChange={e => setFormData({...formData, units: e.target.value})} /></div>
                                        </div>

                                        <div className="grid grid-cols-3 gap-2">
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Início Vendas</label><input type="date" className="settings-input mt-1" value={formData.salesStart} onChange={e => setFormData({...formData, salesStart: e.target.value})} /></div>
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Início Obras</label><input type="date" className="settings-input mt-1" value={formData.constructionStart} onChange={e => setFormData({...formData, constructionStart: e.target.value})} /></div>
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Entrega</label><input type="date" className="settings-input mt-1" value={formData.deliveryDate} onChange={e => setFormData({...formData, deliveryDate: e.target.value})} /></div>
                                        </div>

                                        <div className="grid grid-cols-2 gap-4">
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Status da Obra</label><select className="settings-input mt-1" value={formData.constructionStatus} onChange={e => setFormData({...formData, constructionStatus: e.target.value})}><option>Planta</option><option>Em Construção</option><option>Pronto</option></select></div>
                                            <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Fluxo Pagamento</label><input className="settings-input mt-1" value={formData.paymentPlan} onChange={e => setFormData({...formData, paymentPlan: e.target.value})} placeholder="Ex: 30% obra + financ." /></div>
                                        </div>

                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Link URL da Foto</label><input className="settings-input mt-1" value={formData.image} onChange={e => setFormData({...formData, image: e.target.value})} placeholder="https://..." /></div>
                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Link PDF eBook (Google Drive/Dropbox)</label><input className="settings-input mt-1" value={formData.ebookUrl} onChange={e => setFormData({...formData, ebookUrl: e.target.value})} placeholder="https://..." /></div>
                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Linktree / Site Oficial</label><input className="settings-input mt-1" value={formData.linktree} onChange={e => setFormData({...formData, linktree: e.target.value})} /></div>
                                    </div>
                                )}
                                ...
                                {formData.type === 'agenda' && (
                                    <div className="grid grid-cols-2 gap-4">
                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Data</label><input type="date" className="settings-input mt-1" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} /></div>
                                        <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Hora</label><input type="time" className="settings-input mt-1" value={formData.time} onChange={e => setFormData({...formData, time: e.target.value})} /></div>
                                        <div className="col-span-2"><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Tipo</label><select className="settings-input mt-1" value={formData.obs} onChange={e => setFormData({...formData, obs: e.target.value})}><option>Visita</option><option>Reunião</option><option>Outro</option></select></div>
                                    </div>
                                )}

                                {!editingId && (
                                    <div><label className="text-xs font-bold opacity-50 uppercase ml-1 text-muted">Categoria</label><select className="settings-input mt-1" value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})}><option value="client">Cliente</option><option value="property">Imóvel</option><option value="agenda">Agenda</option></select></div>
                                )}
                            </div>
                        </div>

                        {/* 3. RODAPÉ FIXO */}
                        <div className="p-6 pt-4 flex-shrink-0 flex gap-4 border-t border-slate-200/10 z-10 bg-inherit rounded-b-2xl">
